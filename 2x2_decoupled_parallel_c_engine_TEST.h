...
static inline int hpnt_execute_channel_step(
    ...
    /* Pre-calculation of the shared scalar term for SIMD optimization */
    double k1_r_noise = k1 * r_noise;
    ...
    if ((x0_new != x0_new) || (x1_new != x1_new) || 
        ...
        /* Expand covariance (uncertainty) to force rapid filter adaptation upon recovery */
        state->x0 = x0_pred;
        state->x1 = x1_pred;
        state->p00 = 10.0; 
        state->p01 = 0.0;
