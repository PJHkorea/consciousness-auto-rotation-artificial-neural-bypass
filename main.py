...
# =========================================================================
# CORE FILTER ENGINE (Enhanced Stability & Exact Formula Alignment)
# =========================================================================
...
def _execute_single_step_core(
    ...
    # State Prediction (Linear Vector Rotation)
    ...
    # Pre-update Cauchy-Schwarz Guard to prevent negative matrix singularity
    ...
    # Josephson Form Covariance Update (Mathematically Standardized)
    ...
    # Hard Bounds for Stability (Aligned with Technical Specification at 1e-14)
    if p00_new < 1e-14: p00_new = 1e-14
    if p11_new < 1e-14: p11_new = 1e-14
    
    # Post-update Cauchy-Schwarz Guard (Bound aligned to 1e-28)
    ...
    # Exception Guard (NaN & Overflow Hard Truncation)
    if (math.isnan(x0_new) or math.isnan(x1_new) or
        ...
        not (abs(x0_new) < 1e10 and abs(x1_new) < 1e10 and
             p00_new >= 1e-14 and p00_new < 1e10 and
             abs(p01_new) < 1e10 and
             p11_new >= 1e-14 and p11_new < 1e10)):
        x0_new, x1_new = 0.0, 0.0
        p00_new, p01_new, p11_new = 1.0, 0.0, 1.0

    # Energy Probability Mapping (Target Frequency Amplitude Power Extraction)
    ...
    # Safe Gating Mechanism (Guaranteed non-zero division division)
...
# =========================================================================
# WRAPPER INTERFACE (With Parametric Input Validation Guard)
# =========================================================================
class ScalarJosephKalmanFilter:
    def __init__( self, fs, target_freq, q_noise=1e-3, r_noise=2.25, lambda_val=0.5, theta_gate=0.3):
        ...
        # Guard Clause: Enforce strict boundaries on the gating threshold to prevent analytical breakdown
...
def run_simulation():
    ...
    # Intent Section Simulation (4.0s ~ 7.0s Burst)
    ...
    plt.title('2x2 Pure Scalar Joseph Form Kalman Filter (Robust Enhanced)')
    plt.legend()
    plt.grid(True)
