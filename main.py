...
# 250Hz real-time processing loop with minimized arithmetic operations
for i in range(N):
    # Time Update (Predict)
    x0_minus = cos_t * x0 - sin_t * x1
    x1_minus = sin_t * x0 + cos_t * x1
    
    # Simplification leveraging covariance matrix symmetry (p01 == p10)
    common_term1 = cos_t * p00 - sin_t * p01
    common_term2 = cos_t * p01 - sin_t * p11
    
    p00_m = cos_t * common_term1 - sin_t * common_term2 + q_val
    p01_m = sin_t * common_term1 + cos_t * common_term2
    p11_m = sin_t * (sin_t * p00 + cos_t * p01) + cos_t * (sin_t * p01 + cos_t * p11) + q_val
    ...
    k0 = p00_m / innov_cov
    k1 = p01_m / innov_cov  # Reusing p01_m instead of computing redundant p10_m
    
    v = y - x0_minus
    x0 = x0_minus + k0 * v
    x1 = x1_minus + k1 * v
    
    # Covariance Update & Hard-Symmetrization
    p00 = (1.0 - k0) * p00_m
    p01 = (1.0 - k0) * p01_m
    p10 = p01  # Hard enforcement of structural symmetry (Eliminates truncation error)
    p11 = p11_m - k1 * p01_m
