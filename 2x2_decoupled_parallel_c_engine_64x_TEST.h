#ifndef _SAFE_CRANIAL_CORE_64_H
#define _SAFE_CRANIAL_CORE_64_H
...
/*
 * ==============================================================================
 * [🚨 컴파일러 옵션 및 하이엔드 코어 지침 - 필수 준수]
 * [🚨 Compiler Options & High-End Core Guidelines - MANDATORY]
 *
 ...
 *
 * 2. 64비트 고성능 프로세서 전용 명세 [배포 버전]
 * - 본 헤더 파일은 64비트 고성능 FPU를 탑재한 PC 및 단일 보드 컴퓨터(SBC) 환경에 맞춰 
 *   수학적 완벽성과 극한의 부동소수점 정밀도를 달성하도록 double 규격으로 마감되었습니다.
 *
 * 2. 64-bit High-Performance Processor Specification [Production Release]
 * - This header is optimized for PC and single-board computer (SBC) environments equipped 
 *   with 64-bit hardware FPUs, satisfying strict mathematical integrity using double precision.
 * ==============================================================================
 */
...
// 하드웨어 노이즈 임계값 / Hardware noise upper bound
...
#else
#define HPNT_ABS(x) (((x) < 0.0) ? -(x) : (x))
#define HPNT_ISNAN(x) ((x) != (x))
#endif

/* ──────────────────────────────────────────────────────────────────────────
 * [행렬 프리 SoA 구조체 정의 - double 배포 버전]
 * [Matrix-Free SoA Structure - Double Production Release]
 * ────────────────────────────────────────────────────────────────────────── */
typedef struct {
...
/**
 * @brief 원래 깃허브 구조 그대로 인덱싱 처리하는 단일 스텝 핵심 함수 (64비트 무결성 버전)
 * Processes a single step indexed channel mimicking the original structure with verified 64-bit mathematical integrity.
 */
static inline int hpnt_execute_channel_step_indexed(
    ...
    // 0. 정적 메모리 초기화 로직 / Static Memory Initialization Logic
     ...
     */
    double p01_m = ((cos_sq - sin_sq) * states->p01[ch]) - 
    ...
    /* [No-SQRT Cauchy-Schwarz Guard] 사칙연산 경계면 최적화 완료 / Arithmetic bound optimization complete */
    ...
    if (p01_m_sq > p_prod_m || HPNT_ISNAN(p01_m_sq)) {
        ...
        double min_diag = (p00_m < p11_m) ? p00_m : p11_m;
        p01_m = (p01_m > 0.0) ? min_diag : -min_diag;
    }
     ...
     * 대수적 수식 유도 과정에서 생략되었던 'one_minus_k0' (1 - k0) 가중치를 교차항에 정확하게 결합합니다. (최소 클럭 소모)
     * 
     ...
     * during long-term operation. (Cost: Minimal CPU cycles)
     */
    double p11_new = p11_m - (2.0 * k1 * one_minus_k0 * p01_m) + 
    ...
    if (p01_new_sq > p_prod || HPNT_ISNAN(p01_new_sq)) {
        double min_diag_new = (p00_new < p11_new) ? p00_new : p11_new;
        p01_new = (p01_new > 0.0) ? min_diag_new : -min_diag_new;
    }
    ...
    if (scaled_energy > 0.0 && !HPNT_ISNAN(scaled_energy)) {
        if (scaled_energy >= HPNT_SATURATION_NODE) {
            ...
            /* 정밀 매핑을 위한 Padé [1/1] 유니폴라 계수 정합 / Coefficient mapping for Padé [1/1] */
            double num = 6.0 * scaled_energy;
            double den = 12.0 + (scaled_energy * scaled_energy);
            raw_prob = num / den;
